...
def calculate_euclidean_distance3(input_1,input_2):
    Euclidean_distance = input_1 - input_2
    b = np.uint8(input_1<input_2) * 254 +1
    #print(Euclidean_distance)
    return np.sum(Euclidean_distance * b)  
...
def calculate_nearest_neighbour(trainImages, trainLabels, testImages, testLabels, NumTrain, NumTest):
    ...
    for i in range(NumTest):
        ...
        for k in range(NumTrain):
            dist = calculate_euclidean_distance3(testImages[i],trainImages[k])
...
#cluster_templates, cluster_labels = cluster(train_img,train_labels,64)
...
def calculate_knn(trainImages, trainLabels, testImages, testLabels, NumTrain, NumTest, K):
    ...
    corrPred = [] 
    for i in range(NumTest):
        pred = np.zeros(7)  # New prediction for each testImage in the dataset
        ...
        for j in range(NumTrain):
            ...
            if(dist < np.max(minDist)):
                minDist[np.argmax(minDist)] = dist
                indexPred[np.argmax(minDist)] = j
                pred[np.argmax(minDist)] = trainLabels[j]
        
        digit, count = np.unique(pred, return_counts=True)      #henter ut arrays med hvilke prediction og dens forekomster
        guess = digit[np.argmax(count)]
        guess = int(guess)

        if(guess == testLabels[i]):
            corrPred.append([i,guess])
            #Here we can increment confusion matrix at right indexes
        else:
            wrongPred.append([i,guess])
        confusionMatrix[testLabels[i],guess] += 1
    print("Confusion matrix: \n", confusionMatrix)   
    #-------plot av confusion matrix ---------
    ax = sns.heatmap(confusionMatrix, annot=True, cmap='Greens', linewidths=.7, cbar_kws={"shrink": .82},linecolor='black', clip_on=False)
    ax.set_title('Confusion Matrix with K=7\n\n')
    ax.set_xlabel('\nPredicted digit')
    ax.set_ylabel('Actual digit')
    ...
    ax.set_title('Confusion Matrix given in percentage, with K=7\n\n')
    ax.set_xlabel('\nPredicted digit')
    ax.set_ylabel('Actual digit')
